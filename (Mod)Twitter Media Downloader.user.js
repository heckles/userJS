...
const filename = '{date-time}_twitter_{user-name}(@{user-id})_{status-id}_{file-type}';
...
const TMD = (function () {
  ...
  return {
    ...
    click: async function (btn, status_id, is_exist, index) {
      ...
      let invalid_chars = { '\\': '＼', '\/': '／', '\|': '｜', '<': '＜', '>': '＞', ':': '：', '*': '＊', '?': '？', '"': '＂', '\u200b': '', '\u200c': '', '\u200d': '', '\u2060': '', '\ufeff': '', '🔞': '' };
      let datetime = out.match(/{date-time(-local)?:[^{}]+}/) ? out.match(/{date-time(?:-local)?:([^{}]+)}/)[1].replace(/[\\/|<>*?:"]/g, v => invalid_chars[v]) : 'YYYY-MM-DD hh-mm-ss';
      let info = {};
